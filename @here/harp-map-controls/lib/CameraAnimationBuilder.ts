...
export class CameraAnimationBuilder {
    ...
    static createBowFlyToOptions(
        ...
        const maxAltitude =
            altitude ||
            2 *
                mapView.camera.position.distanceTo(
                    mapView.projection.projectPoint(targetControlPoint.target)
                );

        //calculate two CameraPoses on the maximal altitude in between the start and target points
    ...
    static createOrbitOptions(
        ...
        const headingStep = (360 + 45) / steps;
